...
def predict_and_cluster(opts,mode):
	...
	# if mode == 'overlap1':
	# 	overlap1(news,labels)
	# elif mode == 'overlap2':
	# 	overlap2(news,labels)
...
def dbscan_clustering(X, labels_true):
	# Compute 
...
def gmm(X):
	...
	# 分三群
	gmm = mixture.GMM(n_components=2, covariance_type='diag', random_state=None, thresh=0.01, min_covar=0.001, n_iter=100, n_init=1, params='wmc', init_params='wmc')
	gmm.fit(X)
	# 知道一下每群中心
...
def best_part(FG):
	...
	partition = community.best_partition(FG)

	print (partition)
	print ("partition " ,array(partition.values()))
	...
	for com in set(partition.values()) :
	    ...
	    # print (list_nodes)
	    nx.draw_networkx_nodes(FG, pos, list_nodes, node_size = 400,
	                                node_color = str(count / size))

	nx.draw_networkx_edges(FG, pos, alpha=0.5, with_labels=True) # best_partition
	nx.draw_networkx_labels(FG,pos,font_size=12,font_family='roboto')


	# =================================================
	# modularity
	# modularity = community.modularity(partition,FG)
	# values = [partition.get(node) for node in FG.nodes()]
	# nx.draw_spring(FG, cmap = plt.get_cmap('jet'), node_color = values, node_size=30, with_labels=True)
...
def read_edgelist(crosspoint_m2,nodetype=str):
    adj = defaultdict(set)
    edges = set()
...
def write_edge2cid(e2c,delimiter=","):
    ...
    # comm2edges && comm2nodes
    comm2edges, comm2nodes = [], []
    for cid in sorted(cid2edges.keys()):
        nodes,edges = map(str,cid2nodes[cid]), ["%s,%s" % (ni,nj) for ni,nj in cid2edges[cid]]
        comm2edges.append(edges)
        comm2nodes.append(nodes)
        # print ( " ".join(edges) )
        # print ( " ".join([str(cid)] + nodes) );
    print ()
    print ("comm2edges" , comm2edges)
    print ()
    print ("comm2nodes" , comm2nodes)


def overlap2(news,labels):
	print ("\noverlap method 2")
...
if __name__ == '__main__':
	...
	# if sys.argv[-1] == '--overlap1':
	# 	predict_and_cluster(opts,'overlap1')
	# elif sys.argv[-1] == '--overlap2':
	# 	predict_and_cluster(opts,'overlap2') # no use
	# else: 
	predict_and_cluster(opts,'overlap1')
